...
bool Room::enter(std::weak_ptr<Person> person)
{
	...
	if (canEnter(person))
	{
		...
		if (auto current_room = person_to_enter->room().lock())
			current_room->exit(person_to_enter);
		std::cout << person_to_enter->name() << " has entered room(" << ID << ")" << std::endl;
		_occupants.insert(std::move(person));
...
void Room::exit(std::weak_ptr<Person> person)
{
	...
	if (this == ft::weak_lock(person_to_exit->room()).get() && _occupants.erase(person))
	{
...
bool Classroom::canEnter(std::weak_ptr<Person> person) const
{
	...
	if (!room_course)
	{
		std::cout << person_to_enter->name() << " can't enter because classroom(" << id() << ") has no course" << std::endl;
		return (false);
	}
	if (auto professor = std::dynamic_pointer_cast<Professor>(person_to_enter))
	{
		...
		{
			std::cout << "Professor " << professor->name() << " can't enter classroom(" << id() << 
				") because they don't curate course " << room_course->name() << std::endl;
			return (false);
		}
	}
	else if (auto student = std::dynamic_pointer_cast<Student>(person_to_enter))
	{
		...
		{
			std::cout << "Student " << student->name() << " can't enter classroom(" << id() << ") because they are not subscribed to course " << room_course->name() << std::endl;
			return (false);
		}
	}
	return (false);
...
{
	return (o << "SecretarialOffice("  << static_cast<const Room&>(s) << ")");
}
