...
public:
	...
	void exit(std::shared_ptr<Person> person) { ft::erase(_occupants, person); }
	std::ostream &printOccupants(std::ostream &o = std::cout) const { return (ft::print_shared_ptr_container(o, _occupants)); }
};

std::ostream &operator<<(std::ostream &o, const Room &r)
{
	o << "Room(ID: " << r.id() <<  ", occupants: ";
	return (r.printOccupants(o) << ")");
}
...
std::ostream &operator<<(std::ostream &o, const Classroom &c)
{
	...
	return (ft::print_shared_ptr_member(o, c.current_course()) << ", " << static_cast<const Room&>(c) << ")");
}
...
public:
	...
	std::ostream &printArchivedForms(std::ostream &o = std::cout) const { return (ft::print_shared_ptr_container(o, _archivedForms)); }
};
